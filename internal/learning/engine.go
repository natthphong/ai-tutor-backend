...
4. meaning and every correction reason are Thai. reply is one brief English conversational response; reply_th is its faithful natural Thai translation (not the learner meaning or the answer to your question). weaknesses are stable English category keys for ACTUAL errors (e.g. missing_article, past_tense, subject_verb_agreement), never "none" and never a new key per sentence. Return an empty list if no language errors.
...
var FeedbackSchema = map[string]any{"type": "object", "properties": map[string]any{"transcript": map[string]any{"type": "string"}, "reply": map[string]any{"type": "string"}, "reply_th": map[string]any{"type": "string"}, "meaning": map[string]any{"type": "string"}, "correct": map[string]any{"type": "boolean"}, "audio_clear": map[string]any{"type": "boolean"}, "goal_met": map[string]any{"type": "boolean"}, "pronunciation": map[string]any{"type": "string"}, "retry_sentence": map[string]any{"type": "string"}, "level": map[string]any{"type": "string"}, "weaknesses": map[string]any{"type": "array", "items": map[string]any{"type": "string"}}, "vocabulary": map[string]any{"type": "array", "items": map[string]any{"type": "string"}}, "corrections": map[string]any{"type": "array", "items": map[string]any{"type": "object", "properties": map[string]any{"original": map[string]any{"type": "string"}, "corrected": map[string]any{"type": "string"}, "reason": map[string]any{"type": "string"}, "kind": map[string]any{"type": "string", "enum": []string{"grammar", "natural", "professional"}}}, "required": []string{"original", "corrected", "reason", "kind"}}}}, "required": []string{"transcript", "reply", "reply_th", "meaning", "correct", "audio_clear", "goal_met", "pronunciation", "retry_sentence", "level", "weaknesses", "vocabulary", "corrections"}}
